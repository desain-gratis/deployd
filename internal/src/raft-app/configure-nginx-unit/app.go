...
const (
	...
	Command_Host_ResponseUpdateNginxUnitConfig Command = "host.response-update-nginx-unit-config"
...
type RaftApp struct {
	...
	jobCache   *expirable.LRU[jobKey, *entity.JobUpdateNginxUnit]
	jobUsecase *mycontent_base.Handler[*entity.JobUpdateNginxUnit]
}
...
func New(topic notifier.Topic, dbJob *badger.DB) *RaftApp {
	...
	jobUsecase := mycontent_base.New[*entity.JobUpdateNginxUnit](jobStorage)
	jobCache := expirable.NewLRU[jobKey, *entity.JobUpdateNginxUnit](256, nil, 20*time.Minute) // at least until the DB can catch up
...
func (m *RaftApp) GetJobStore() *mycontent_base.Handler[*entity.JobUpdateNginxUnit] {
	return m.jobUsecase
}
...
func (m *RaftApp) OnUpdateV2(ctx context.Context, entry raft.EntryV2) (any, error) {
	...
	switch cmd.Name {
	case Command_User_RequestUpdateNginxUnitConfig:
		// start create job
		...
		return result()
	case Command_Host_ResponseUpdateNginxUnitConfig:
		payload, err := parseAs[entity.SubmitDeploymentJobRequest](cmd.Value)
		if err != nil {
			return nil, fmt.Errorf("%w: failed to parse command as JSON (%v)", err, string(cmd.Value))
		}
...
func (m *RaftApp) userSubmitJob(ctx context.Context, request entity.SubmitDeploymentJobRequest) (ApplyResult, error) {
	...
	return func() (any, error) {
		m.topic.Broadcast(context.Background(), UpdateNginxUnitConfigResponse{})
		return "", nil
	}, nil
}

func (m *RaftApp) hostResponse(ctx context.Context, request entity.SubmitDeploymentJobRequest) (ApplyResult, error) {

	return func() (any, error) {
		m.topic.Broadcast(context.Background(), "TEST")
		return "", nil
	}, nil
}
...
// todo: refac fac
func (m *RaftApp) getJobByID(ctx context.Context, namespace, service, id string) (*entity.JobUpdateNginxUnit, error) {
	// check cache
